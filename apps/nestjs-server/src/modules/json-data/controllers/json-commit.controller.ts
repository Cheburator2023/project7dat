...
export class JsonCommitController {
    ...
    @ApiOperation({
        summary: "Получение сгруппированных коммитов (оригиналы с пользовательскими версиями)",
        description: "Возвращает оригинальные коммиты с их пользовательскими версиями"
    })
    @ApiResponse({
        status: 200,
    ...
    async getGroupedCommits(): Promise<{ commits: Array<{ original: JsonCommitResponseDto, user_versions: JsonCommitResponseDto[] }> }> {
        ...
        const groupedCommits = await Promise.all(
            originalCommits.map(async (original) => {
                const userVersions = await this.jsonCommitService.getUserVersionsByOriginal(original.commit_id);
                return {
                    original: this.mapEntityToDto(original),
                    user_versions: userVersions.map(version => this.mapEntityToDto(version))
                };
            })
        );
    ...
    @ApiOperation({
        ...
        description: "Возвращает все коммиты во всех статусах согласно документации"
    })
    @ApiResponse({
        status: 200,
    ...
    async saveCommit(@Body() saveDto: JsonCommitSaveRequestDto): Promise<JsonCommitResponseDto> {
    ...
    @ApiOperation({
        ...
        description: "Изменение статуса всех коммитов в статусе 'processing' на 'done'"
    })
    ...
    async updateCommitStatus(@Body() statusDto: JsonCommitStatusUpdateDto): Promise<{ updated: number }> {
    ...
    @ApiOperation({
        ...
        description: "Возвращает оригинальный коммит и все его пользовательские версии"
    })
    @ApiParam({
        name: "parentId",
    ...
    async getCommitsByParent(@Param("parentId") parentId: string): Promise<JsonCommitListResponseDto> {
    ...
    @ApiOperation({
        ...
        description: "Удаление пользовательской версии коммита (оригинал удалить нельзя)"
    })
    @ApiParam({
        name: "id",
    ...
    @ApiResponse({
        status: 400,
        description: "Нельзя удалить оригинальный коммит или коммит в статусе 'done'"
    })
    ...
    async getCommitsByType(@Param("type") type: CommitType): Promise<JsonCommitListResponseDto> {
