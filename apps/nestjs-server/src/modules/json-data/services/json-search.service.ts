...
export class JsonSearchService {
	 ...
	 */
	async getJson({search}: {search: string}): Promise<JsonExportResponseDto> {
		this.logger.log(
		...
		try {
			...
			const entitiesWithDetails = await this.getEnhancedEntitiesWithDetails(search);

			const results: any[] = [];
			for (const entity of entitiesWithDetails) {
				const mappingForEntity = await this.exportEntityRelations(entity.full_name);
	 ...
	 */
	private async getEnhancedEntitiesWithDetails(search?: string): Promise<EntityWithDetails[]> {
		const query = `
            SELECT
                e.entity_id,
                e.full_name,
