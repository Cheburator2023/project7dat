...
export class JsonImportController {
	...
	@ApiHeader({
		name: "x-user",
		description:
			'Идентификатор пользователя в формате: { "id": "user-id", "username": "user-name", "email": "user@example.com" }',
		required: true,
		schema: {
			type: "string",
			example:
				'{"id":"12345","username":"ivanov","email":"ivanov@company.com"}',
		},
	})
	@ApiHeader({
		name: "x-user",
	...
	async importSurmJson(
		...
		try {
			...
			if (!validationReport.summary.isValid) {
				this.logger.warn(
					`Валидация не пройдена: ${JSON.stringify(validationReport.validation.errors)}`,
				);
			...
			// Нормализация данных
			processedData =
				this.jsonValidationService.normalizeJsonData(processedData);

			// Проверка на конфликты
			const conflictCheck = await this.jsonMappingService.checkAffectedMarts(
	...
	async importDappJson(
		...
		try {
			...
			if (!validationReport.summary.isValid) {
				this.logger.warn(
					`Валидация DAPP JSON не пройдена: ${JSON.stringify(validationReport.validation.errors)}`,
				);
				throw new BadRequestException({
					message: "Валидация JSON не пройдена",
					details: validationReport,
			...
			const dappValidation = this.validateDappSpecificFields(
				importRequest.data,
			);
			...
			// Нормализация данных
			const processedData = this.jsonValidationService.normalizeJsonData(
				importRequest.data,
			);
	...
	async comprehensiveValidation(
		@Body() importRequest: JsonImportRequestDto,
	): Promise<ComprehensiveValidationResponse> {
		...
		try {
			...
			const schemaVersionCheck =
				this.jsonValidationService.validateSchemaVersion(importRequest.data);
	 ...
	 */
	private validateDappSpecificFields(data: any): {
		isValid: boolean;
		errors: string[];
	} {
		const errors: string[] = [];
	...
	private hasUnmatchedEntities(data: any): boolean {
		...
		return data.mappings.some(
			(mapping: any) =>
				mapping.unmatched &&
				Array.isArray(mapping.unmatched) &&
	...
	private generateRecommendations(
		...
		if (versionCompatibility.migrationRequired) {
			recommendations.push(
				`Требуется миграция данных с версии ${validationReport.summary.schemaVersion}`,
			);
		}
