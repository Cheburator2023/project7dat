...
@ApiTags("JSON Данные (управление документами)")
...
export class JsonDataController {
	...
	@ApiBody({
		...
		schema: {
			...
			properties: {
				...
				system_code: {
					type: "string",
					description: "Код системы (опционально, по умолчанию 1642)",
					example: "1642",
				},
			},
			required: ["data"],
	...
	@ApiResponse({
		...
		schema: {
			...
			properties: {
				...
				system_code: { type: "string", example: "1642" },
	...
	@ApiQuery({
		...
		required: false,
		type: String,
		description: "Фильтр по коду системы",
		example: "1642",
	})
	@ApiResponse({
		status: 200,
	...
	/** @deprecated Используйте GET /json-export/dl или /json-export/dl/paginated */
	@ApiExcludeEndpoint()
	@Get("current")
	@RealmRole(Permission.DL_VIEW_JSON_DATA)
	@ApiOperation({
		summary: "[DEPRECATED] Получить последний JSON документ",
		description:
			"DEPRECATED: Используйте GET /json-export/dl для полного графа " +
			"или GET /json-export/dl/paginated для пагинированной загрузки. " +
			"Этот эндпоинт возвращает JSON-документ из таблицы json_data (не из РБД).",
		deprecated: true,
	})
	@ApiResponse({
		status: 200,
	...
	/** @deprecated Используйте GET /json-export/dl/paginated для списка сущностей */
	@ApiExcludeEndpoint()
	@Get(":id")
	@RealmRole(Permission.DL_VIEW_JSON_DATA)
	@ApiOperation({
		summary: "[DEPRECATED] Получить JSON документ по ID",
		description:
			"DEPRECATED: Для чтения графа используйте /json-export/dl. " +
			"Этот эндпоинт нужен только для управления JSON-документами.",
		deprecated: true,
	})
	@ApiParam({
		name: "id",
