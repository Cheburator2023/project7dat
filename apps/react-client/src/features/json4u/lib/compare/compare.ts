...
import { MaxEditLength, arrayDiff, myersDiff } from "./myers";

export function compareJSON(ltree: Tree, rtree: Tree): DiffPair[] {
	const Diff = (node: Node, type: DiffType, includeBound = true) => {
		return includeBound
			? newDiff(node.boundOffset, node.boundLength, type)
			: newDiff(node.offset, node.length, type);
	};
	...
	const comparer = {
		...
		diffArray(lnode: Node, rnode: Node) {
			...
			for (let i = 0; i < n; i++) {
				...
				if (delDiff && insDiff && l === r) {
					this.diff(
						ltree.getChild(lnode, String(l))!,
						rtree.getChild(rnode, String(r))!,
					);
				} else {
					pairs.push({
						left: delDiff
							? Diff(ltree.getChild(lnode, String(l))!, "del")
							: undefined,
						right: insDiff
							? Diff(rtree.getChild(rnode, String(r))!, "ins")
							: undefined,
					});
				}
			}
		...
		diffObject(lnode: Node, rnode: Node) {
			const { intersection, leftOnly, rightOnly } = splitKeys(
				getChildrenKeys(lnode),
				getChildrenKeys(rnode),
			);
...
if (import.meta.vitest) {
	...
	it("arrayDiff", () => {
		expect(compareArray(["a", "b", "c"], ["a", "c", "d"])).toMatchObject([
			newDiff(1, 1, "del"),
			newDiff(2, 1, "ins"),
		]);
	});
