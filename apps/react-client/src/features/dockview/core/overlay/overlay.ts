				...
				move.value = new CompositeDisposable(
					addDisposableListener(window, "pointermove", (e) => {
						...
						const moveTop = () => {
							...
							const maxTop =
								startPosition!.originalY + startPosition!.originalHeight >
								containerRect.height
									? Math.max(0, containerRect.height - Overlay.MINIMUM_HEIGHT)
									: Math.max(
											0,
							...
							height =
								startPosition!.originalY + startPosition!.originalHeight - top;
						...
						const moveLeft = () => {
							const maxLeft =
								...
								containerRect.width
									? Math.max(0, containerRect.width - Overlay.MINIMUM_WIDTH) // Prevent extending beyong right edge
									: Math.max(
											0,
							...
							width =
								startPosition!.originalX + startPosition!.originalWidth - left;
						...
						const moveRight = () => {
							...
							const minWidth =
								left < 0 &&
								typeof this.options.minimumInViewportWidth === "number"
