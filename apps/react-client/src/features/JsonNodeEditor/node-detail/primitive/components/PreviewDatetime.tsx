...
import { type Key, memo, useEffect, useState } from "react";
import { Calendar } from "./Calendar";
import {
	PROPERTY_VALUE_TABLE_COLUMNS,
	PropertyValueTableColumnKey,
} from "./PropertyValueTable";
			...
			>
				...
				<TableBody items={rows}>
					{(row) => (
						<TableRow
							key={row.property}
							className="[&+&]:border-t-1 [&+&]:border-solid [&+&]:border-t-default-200"
						>
							{(columnKey: Key) => {
								const isPropertyColumn =
									columnKey === PropertyValueTableColumnKey.Property;
								const isValueColumn =
									columnKey === PropertyValueTableColumnKey.Value;
									...
									<TableCell
										className={
											isPropertyColumn
												...
												: "text-xs font-normal text-default-600"
										}
									>
										{row.property === "Relative" && isValueColumn ? (
											<RelativeTimeFormatter
												unixTimestamp={
													row[
														columnKey as PropertyValueTableColumnKey
													] as number
												}
											/>
										) : (
