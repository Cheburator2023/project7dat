...
const _UriTable = ({ httpUri }: Props) => {
	const getUriTableRows = useCallback(
		(httpUri: HttpUri): PropertyValueTableRow[] => {
			const httpUrlObject: URL = new URL(httpUri);

			return DISPLAY_TARGET_URL_PROPERTIES.filter(
	...
	return (
		<PropertyValueTable rows={getUriTableRows(httpUri)} ariaLabel="URL table" />
	);
};
