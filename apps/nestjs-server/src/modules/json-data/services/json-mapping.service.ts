...
export class JsonMappingService {
    ...
    async importJsonData(importRequest: JsonImportRequestDto): Promise<{
        ...
        if (!validated) {
            throw new ConflictException("JSON должен быть проверен и подтвержден пользователем перед импортом");
        }

        // Комплексная валидация JSON
        const validationReport = this.jsonValidationService.generateValidationReport(data);
        ...
        if (recursionCheck.hasRecursion) {
            throw new BadRequestException(`Обнаружены рекурсивные зависимости: ${JSON.stringify(recursionCheck.cycles)}`);
        }

        // Проверка на дублирование
        const duplicateCheck = this.checkForDuplicates(processedData);
        if (duplicateCheck.hasDuplicates) {
            throw new BadRequestException(`Обнаружены дубликаты: ${duplicateCheck.duplicates.join(", ")}`);
        }

        // Проверка зависимостей для модифицированных витрин
        const modifiedEntities = (processedData.entities || []).filter((entity: any) => entity.modified);
        ...
        try {
            ...
            const changeId = await this.createChangeRecord(processedData, user, changeName, queryRunner);
            this.logger.log(`Создана запись изменения с ID: ${changeId}`);

            // Шаг 2: Обработка процесса
            const process = await this.handleProcess(processedData.desc, changeId, queryRunner);
            this.logger.log(`Обработан процесс: ${process.name} (ID: ${process.process_id})`);

            // Шаг 3: Обработка сущностей
            const entitiesStats = await this.handleEntities(processedData.entities, changeId, queryRunner);
            this.logger.log(`Обработано сущностей: ${entitiesStats.count}, атрибутов: ${entitiesStats.attributesCount}`);

            // Шаг 4: Обработка маппингов
            const mappingsStats = await this.handleMappings(processedData.mappings, process.process_id, changeId, queryRunner);
     ...
     * Шаг 2: Обработка процесса
     */
    private async handleProcess(desc: any, changeId: number, queryRunner: QueryRunner): Promise<ProcessEntity> {
        ...
        if (process) {
            this.logger.log(`Найден существующий процесс: ${processName} (ID: ${process.process_id})`);
     ...
     * Шаг 3: Обработка сущностей
     */
    private async handleEntities(entities: any[], changeId: number, queryRunner: QueryRunner): Promise<{ count: number; attributesCount: number }> {
        ...
        for (const entityData of entities) {
            const entityAttributesCount = await this.handleSingleEntity(entityData, changeId, queryRunner);
     ...
     */
    private async handleSingleEntity(entityData: any, changeId: number, queryRunner: QueryRunner): Promise<number> {
        // Валидация типа сущности
        const isValidType = await this.entityTypeService.validateEntityType(entityData.type);
        if (!isValidType) {
            this.logger.warn(`Неизвестный тип сущности: ${entityData.type} для ${entityData.id}`);
        }
        ...
        if (!entity) {
            ...
            entity.entity_type_id = await this.entityTypeService.mapJsonTypeToEntityType(entityData.type);
            entity.entity_container_id = await this.entityContainerService.resolveEntityContainer(
        ...
        if (entityData.attrSeq && Array.isArray(entityData.attrSeq)) {
            for (const attrData of entityData.attrSeq) {
                await this.handleAttribute(attrData, entity.entity_id, changeId, queryRunner);
                attributesCount++;
            }
        }
     ...
     * Обработка атрибута
     */
    private async handleAttribute(attrData: any, entityId: number, changeId: number, queryRunner: QueryRunner): Promise<void> {
        // Валидация типа атрибута
        const typeId = await this.attributeTypeService.resolveAttributeTypeFromJson(attrData.type);
        ...
        if (!existingAttribute) {
            this.logger.log(`Создание нового атрибута: ${attrData.name} для сущности ${entityId}`);
            ...
            this.logger.log(`Атрибут уже существует: ${attrData.name} для сущности ${entityId}`);
     ...
     * Шаг 4: Обработка маппингов
     */
    private async handleMappings(mappings: any[], processId: number, changeId: number, queryRunner: QueryRunner): Promise<{ count: number }> {
     ...
     */
    private async handleSingleMapping(mapping: any, processId: number, changeId: number, queryRunner: QueryRunner): Promise<void> {
        ...
        if (!targetEntity) {
            throw new NotFoundException(`Таргет сущность не найдена: ${mapping.entityId}`);
        }
        ...
        const savedEntityMap = await queryRunner.manager.save(EntityMapEntity, entityMap);

        // Обработка зависимостей
        if (mapping.deps && Array.isArray(mapping.deps)) {
            for (const dep of mapping.deps) {
                await this.handleDependency(dep, savedEntityMap.entity_map_id, targetEntity.entity_id, changeId, queryRunner);
            }
        }
    }
     ...
     */
    private async handleDependency(dep: any, entityMapId: number, targetEntityId: number, changeId: number, queryRunner: QueryRunner): Promise<void> {
        ...
        if (!sourceEntity) {
            throw new NotFoundException(`Source сущность не найдена: ${dep.entityId}`);
        }

        // Обработка attrMaps
        if (dep.attrMaps && Array.isArray(dep.attrMaps)) {
            for (const attrMap of dep.attrMaps) {
                await this.handleAttrMap(attrMap, entityMapId, sourceEntity.entity_id, targetEntityId, changeId, queryRunner);
            }
        }
        ...
        if (dep.atrDeps && Array.isArray(dep.atrDeps)) {
            for (const attrDep of dep.atrDeps) {
                await this.handleAttrDep(attrDep, entityMapId, sourceEntity.entity_id, changeId, queryRunner);
            }
        }
    }
     ...
     * Обработка attrMap
     */
    private async handleAttrMap(attrMap: any, entityMapId: number, sourceEntityId: number, targetEntityId: number, changeId: number, queryRunner: QueryRunner): Promise<void> {
        ...
        if (!sourceAttribute) {
            throw new NotFoundException(`Source атрибут не найден: ${attrMap.src} в сущности ${sourceEntityId}`);
        }
        ...
        if (!targetAttribute) {
            throw new NotFoundException(`Target атрибут не найден: ${attrMap.dst} в сущности ${targetEntityId}`);
        }
        ...
        const savedAttributeMap = await queryRunner.manager.save(AttributeMapEntity, attributeMap);
        ...
        await queryRunner.manager.save(AttributeMapSourceEntity, attributeMapSource);
    }

    /**
     * Обработка attrDep
     */
    private async handleAttrDep(attrDep: any, entityMapId: number, sourceEntityId: number, changeId: number, queryRunner: QueryRunner): Promise<void> {
        ...
        if (!sourceAttribute) {
            throw new NotFoundException(`Source атрибут не найден: ${attrDep.attr} в сущности ${sourceEntityId}`);
        }
        ...
        if (attrDep.linkTypes && Array.isArray(attrDep.linkTypes)) {
            for (const linkType of attrDep.linkTypes) {
                ...
                await queryRunner.manager.save(EntityAttributeMapEntity, entityAttributeMap);
            }
        }
    }
     ...
     */
    private async cleanupExistingMappings(processId: number, queryRunner: QueryRunner): Promise<void> {
        ...
        if (entityMaps.length > 0) {
            ...
            await queryRunner.manager.delete(AttributeMapSourceEntity, {
                attribute_map_id: In(
                    await this.attributeMapRepository.find({
     ...
     */
    async checkAffectedMarts(entities: any[]): Promise<{ hasConflicts: boolean; conflicts: string[] }> {
        ...
        for (const entity of modifiedEntities) {
            const usageCheck = await this.dependencyCheckService.checkMartUsage([entity.id]);
            if (usageCheck.hasConflicts) {
                usageCheck.conflicts.forEach(conflict => {
                    conflicts.push(`Сущность ${conflict.entityName} используется в процессах: ${conflict.processes.join(', ')}`);
                });
            }
        }
     ...
     */
    checkForRecursion(entities: any[], mappings: any[]): { hasRecursion: boolean; cycles: string[][] } {
     ...
     */
    checkForDuplicates(data: any): { hasDuplicates: boolean; duplicates: string[] } {
