...
export class JsonValidationService {
     ...
     */
    validateJsonForImport(data: any): { isValid: boolean; errors: string[]; warnings: string[] } {
        const errors: string[] = [];
        ...
        if (jsonSize > this.maxJsonSize) {
            throw new BadRequestException(`Размер JSON превышает лимит: ${jsonSize} > ${this.maxJsonSize}`);
        }
        ...
        if (data.entities && data.entities.length > this.maxEntities) {
            errors.push(`Превышено максимальное количество сущностей: ${data.entities.length} > ${this.maxEntities}`);
        }
        ...
        if (data.entities && Array.isArray(data.entities)) {
            data.entities.forEach((entity: any, index: number) => {
                ...
                if (entity.modified === undefined) {
                    warnings.push(`Сущность ${index}: отсутствует флаг modified (будет установлен в false)`);
                }
                ...
                if (entity.attrSeq && Array.isArray(entity.attrSeq)) {
                    if (entity.attrSeq.length > this.maxAttributes) {
                        errors.push(`Сущность ${entity.id}: превышено максимальное количество атрибутов: ${entity.attrSeq.length} > ${this.maxAttributes}`);
                    }

                    entity.attrSeq.forEach((attr: any, attrIndex: number) => {
                        if (!attr.name) {
                            errors.push(`Сущность ${entity.id}, атрибут ${attrIndex}: отсутствует name`);
                        }
                        if (!attr.type) {
                            errors.push(`Сущность ${entity.id}, атрибут ${attrIndex}: отсутствует type`);
                        }

                        // Проверка корректности типа атрибута
                        const validAttrTypes = ['timestamp', 'decimal', 'string', 'integer'];
                        if (attr.type && !validAttrTypes.includes(attr.type)) {
                            errors.push(`Сущность ${entity.id}, атрибут ${attrIndex}: неверный тип '${attr.type}'`);
                        }
                    });
                }
            });
            ...
            data.mappings.forEach((mapping: any, index: number) => {
                ...
                if (mapping.deps && Array.isArray(mapping.deps)) {
                    mapping.deps.forEach((dep: any, depIndex: number) => {
                        if (!dep.entityId) {
                            errors.push(`Маппинг ${index}, зависимость ${depIndex}: отсутствует entityId`);
                        }

                        const sourceEntity = data.entities.find((e: any) => e.id === dep.entityId);
                        if (!sourceEntity) {
                            warnings.push(`Маппинг ${index}, зависимость ${depIndex}: source entity не найдена: ${dep.entityId}`);
                        }
                        ...
                        if (dep.attrMaps && Array.isArray(dep.attrMaps)) {
                            dep.attrMaps.forEach((attrMap: any, attrMapIndex: number) => {
                                if (!attrMap.src) {
                                    errors.push(`Маппинг ${index}, зависимость ${depIndex}, attrMap ${attrMapIndex}: отсутствует src`);
                                }
                                if (!attrMap.dst) {
                                    errors.push(`Маппинг ${index}, зависимость ${depIndex}, attrMap ${attrMapIndex}: отсутствует dst`);
                                }
                            });
                        }
                        ...
                        if (dep.atrDeps && Array.isArray(dep.atrDeps)) {
                            dep.atrDeps.forEach((attrDep: any, attrDepIndex: number) => {
                                if (!attrDep.attr) {
                                    errors.push(`Маппинг ${index}, зависимость ${depIndex}, attrDep ${attrDepIndex}: отсутствует attr`);
                                }
                                if (!attrDep.linkTypes || !Array.isArray(attrDep.linkTypes)) {
                                    warnings.push(`Маппинг ${index}, зависимость ${depIndex}, attrDep ${attrDepIndex}: отсутствуют linkTypes`);
                                }
                            });
                        }
                    });
        ...
        if (data.entities && Array.isArray(data.entities)) {
            data.entities.forEach((entity: any, index: number) => {
                if (!this.isValidName(entity.name)) {
                    errors.push(`Сущность ${index}: имя содержит недопустимые символы: ${entity.name}`);
                }

                if (entity.attrSeq && Array.isArray(entity.attrSeq)) {
                    entity.attrSeq.forEach((attr: any, attrIndex: number) => {
                        if (!this.isValidName(attr.name)) {
                            errors.push(`Сущность ${index}, атрибут ${attrIndex}: имя содержит недопустимые символы: ${attr.name}`);
                        }
                    });
                }
            });
        ...
        this.logger.log(`Валидация завершена. Ошибок: ${errors.length}, предупреждений: ${warnings.length}`);
    ...
    generateValidationReport(data: any): {
        ...
        const normalizedData = this.normalizeJsonData(data);

        // Статистика
        const statistics = {
            entitiesCount: data.entities?.length || 0,
            attributesCount: data.entities?.reduce((acc: number, entity: any) =>
                acc + (entity.attrSeq?.length || 0), 0) || 0,
            mappingsCount: data.mappings?.length || 0,
            dependenciesCount: data.mappings?.reduce((acc: number, mapping: any) =>
                acc + (mapping.deps?.length || 0), 0) || 0,
            modifiedEntitiesCount: data.entities?.filter((e: any) => e.modified).length || 0
        };

        return {
            summary: {
                isValid: validation.isValid && integrity.isValid && schemaVersion.isValid,
            ...
            if (entity.attrSeq && Array.isArray(entity.attrSeq)) {
                ...
                entity.attrSeq.forEach((attr: any) => {
                    if (attrNames.has(attr.name)) {
                        duplicates.push(`Дублирование атрибута: ${attr.name} в сущности ${entity.id}`);
                    }
