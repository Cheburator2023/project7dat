...
export const useCurrentDataLineageGraph = (options?: { enabled?: boolean, search?:string }) => {
	const { initializeGraph, setCurrentGraphId, currentGraphId  } = useDataLineageStore();
	return useQuery({
		...
		queryFn: async () => {
			try {
				...
				if(options?.search && !currentGraphId){
					const backendItem = await jsonDataService.getSearchEntity(options?.search.toUpperCase());
