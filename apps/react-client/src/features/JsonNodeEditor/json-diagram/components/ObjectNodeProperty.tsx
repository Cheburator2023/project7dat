...
const _ObjectNodeProperty = ({
	...
	return (
		<div className="relative flex h-nodeContentHeight items-center justify-between pr-2 [&+&]:border-t-1 [&+&]:border-solid [&+&]:border-t-default-200">
			...
			{isNumberData && (
				<Text className="overflow-hidden text-ellipsis whitespace-nowrap text-success-600">
					{propertyV}
				</Text>
			)}
			...
			{hasChildNode && (
				<DefaultHandle
					style={{ backgroundColor: semanticColors[theme].default[300] }}
					id={propertyK}
					type="source"
				/>
			)}
