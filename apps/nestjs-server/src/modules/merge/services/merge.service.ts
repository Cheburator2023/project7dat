...
export class MergeService {
	...
	constructor(
		...
		readonly _commitRepository: Repository<JsonCommitEntity>,
	...
	async confirmMerge(
		...
		try {
			...
			const _importResult = await this.jsonImportService.importJsonData({
			...
			this.logger.error(
				`Ошибка при подтверждении слияния: ${error.message}`,
				error.stack,
			);
			throw new InternalServerErrorException(
				"Не удалось сохранить смерженную модель",
			);
		} finally {
	...
	private performMerge(
		...
		const targetEntities = commitJson.entities.filter(
			(e: any) => e.modified === true,
		);
		for (const targetEntity of targetEntities) {
			const index = merged.entities.findIndex(
				(e: any) => e.id === targetEntity.id,
			);
			if (index !== -1) {
