...
export class S2tExcelParserService {
	...
	parseExcel(buffer: Buffer, filename: string): ParsedS2TData {
		...
		if (rows.length === 0)
			throw new BadRequestException("Нет данных на листе Mapping");
		...
		if (headerIndex === -1) {
			throw new BadRequestException(
				"Не найдено непустых строк на листе Mapping",
			);
		}
		...
		for (let i = dataStartIndex; i < rows.length; i++) {
			...
			const hasData = !!(
				parsedRow.sourceAttributeCode ||
				parsedRow.targetAttributeCode ||
				parsedRow.sourceTable ||
				parsedRow.targetTable
			);
			if (!hasData) continue;
		...
		this.logger.log(
			`Распарсено ${parsedRows.length} строк, тип файла: ${fileType}`,
		);
