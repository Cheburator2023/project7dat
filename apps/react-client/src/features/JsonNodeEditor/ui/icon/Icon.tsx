...
const _Icon = ({ icon, size, color = "#000000", style, onClick }: Props) => {
	return (
		<span
			...
			onKeyUp={
				isFunction(onClick)
					? (e) => e.key === "Enter" && onClick?.(e as any)
					: undefined
			}
			tabIndex={isFunction(onClick) ? 0 : undefined} // Ensure the element is focusable for keyboard interaction
		>
			<svg
				width={size}
				height={size}
