...
import type { InferredDataType } from "../enums/inferred-data-type.enum";
...
const _InferredDetailCard = (
	...
	const renderPropertyKeyChips = useCallback(() => {
		return propertyKeys.map(
			(propertyK: string, index: number, selfArray: string[]) => {
...
export const InferredDetailCard = memo(
	forwardRef<HTMLDivElement, Props>(_InferredDetailCard),
);
