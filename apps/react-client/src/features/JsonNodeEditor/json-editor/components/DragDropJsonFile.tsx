...
const _DragDropJsonFile = ({ afterFileReadSuccess }: Props) => {
	...
	const { dropzoneRef, isDragging, handleFileInputChange } =
		useDragDropJsonFile(afterFileReadSuccess);
	...
	return (
		<div className="relative h-[200px] w-full max-w-full">
			...
			<label
				ref={dropzoneRef}
				className={
					isDragging ? dropzoneClassName.dragging : dropzoneClassName.normal
				}
				htmlFor={fileInputId}
			>
				<Icon
					...
					color={
						isDragging
							...
							: semanticColors[theme].default[500]
					}
				/>
				<Text
					className={
						isDragging ? "text-sm text-success" : "text-sm text-default-800"
					}
				>
					{isDragging
						? "Drop it to import"
						: "Drop a your JSON file, or click here"}
				</Text>
			</label>
		</div>
	);
