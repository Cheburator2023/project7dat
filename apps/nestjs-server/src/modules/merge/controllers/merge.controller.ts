...
export class MergeController {
    ...
    @ApiOperation({
        ...
        description: 'Выполняет слияние коммита с текущей моделью данных и возвращает смерженный JSON и diff',
    })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({ status: 400, description: 'Коммит не в статусе processing или отсутствует JSON' })
    @ApiResponse({ status: 404, description: 'Коммит не найден' })
    async applyMerge(@Body() dto: ApplyMergeRequestDto): Promise<ApplyMergeResponseDto> {
        if (!dto.commitId) {
    ...
    @ApiOperation({
        ...
        description: 'Сохраняет смерженную модель в РБД, создает снепшот и меняет статус коммита',
    })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({ status: 404, description: 'Активная сессия слияния не найдена' })
    async confirmMerge(@Body() dto: ConfirmMergeRequestDto): Promise<ConfirmMergeResponseDto> {
    ...
    @ApiResponse({ status: 404, description: 'Активная сессия слияния не найдена' })
    async cancelMerge(@Body() dto: CancelMergeRequestDto): Promise<CancelMergeResponseDto> {
    ...
    @ApiOperation({
        ...
        description: 'Возвращает данные активной сессии слияния по её идентификатору',
    })
