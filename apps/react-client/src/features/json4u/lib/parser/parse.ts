import { toPointer } from "@react-client/features/json4u/lib/idgen/pointer";
...
export function parseJSON(
	...
	const { nodeMap, nestNodeMap, parseErrors } = doParseJSON(
		text,
		options,
		parentMeta,
	);
...
class Visitor {
	...
	genPath(pathSupplier: () => jsonc.JSONPath) {
		return this.parentMeta.path.length
			? [...this.parentMeta.path, ...pathSupplier()]
			: pathSupplier();
	}

	newNode(path: jsonc.JSONPath, type: NodeType, offset: number, length = 0) {
	...
	onLiteralValue(
		...
		if (this.options?.nest && maybeIterable(value)) {
			const { root, nodeMap, parseErrors } = doParseJSON(value, this.options, {
				path,
			});
			node = root;
			errors = parseErrors;
