...
export class CreateEnhancedJsonViews1760000000027 implements MigrationInterface {
    name = 'CreateEnhancedJsonViews1760000000027';

    public async up(queryRunner: QueryRunner): Promise<void> {
        // 1. Удаляем старые VIEW и создаем новые, улучшенные
        await queryRunner.query(`DROP VIEW IF EXISTS mapping_dependencies_view CASCADE`);
        await queryRunner.query(`DROP VIEW IF EXISTS entity_system_view CASCADE`);
        await queryRunner.query(`DROP VIEW IF EXISTS entity_map_extended_view CASCADE`);

        // 2. Улучшенный VIEW для системы сущностей с правильными кодами
        ...
        // 3. VIEW для маппингов с правильными источниками
        await queryRunner.query(`
        ...
        // 4. VIEW для источников с полной информацией
        await queryRunner.query(`
            ...
            SELECT DISTINCT
                ...
                c_proc.change_date AS source_process_change_date  -- Здесь получаем дату процесса
            ...
            LEFT JOIN changes c_proc ON p.change_id = c_proc.change_id  -- ИСПРАВЛЕНО: change_id = change_id
            WHERE ems.change_id IS NOT NULL;
        `);

        // 5. VIEW для атрибутных маппингов с информацией о системах
        await queryRunner.query(`
        ...
        // 6. VIEW для зависимостей атрибутов с информацией о системах
        await queryRunner.query(`
        ...
        // 7. Комплексный VIEW для формирования JSON в новой структуре
        await queryRunner.query(`
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        // Восстанавливаем старые VIEW (если нужно)
        // ... (код восстановления старых VIEW при необходимости)
    }
}