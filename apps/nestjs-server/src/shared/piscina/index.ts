...
import { kQueueOptions, kTransferable, kValue, kWorkerData } from "./symbols";
import {
	TaskQueue,
	isTaskQueue,
	ArrayTaskQueue,
...
import { AbortSignalAny, AbortSignalEventTarget, AbortError } from "./abort";
import { PiscinaHistogram, PiscinaHistogramHandler } from "./histogram";
...
class ThreadPool {
	...
	constructor(publicInterface: Piscina, options: Options) {
		...
		this.balancer =
			this.options.loadBalancer ??
			LeastBusyBalancer({
				maximumUsage: this.options.concurrentTasksPerWorker,
			});
	...
	_addNewWorker(): void {
		...
		workerInfo.onPortClose(() => {
			workerInfo.workerRef();
		});
	...
	_onError(
		...
		err: Error,
		onlyErrorUnfinishedTasks: boolean,
	) {
	...
	_onWorkerAvailable(workerInfo: WorkerInfo): void {
		...
		if (this.options.idleTimeout === Number.POSITIVE_INFINITY) {
			return;
		}

		// If more workers than minThreads, we can remove idle workers
		if (
			workerInfo.currentUsage() === 0 &&
	...
	runTask(task: any, options: RunOptions): Promise<any> {
		...
		const workers = [...this.workers.readyItems].map(
			(workerInfo) => workerInfo.interface,
		);
		const distributed = this._distributeTask(taskInfo, workers);
...
> extends EventEmitterAsyncResource {
	...
	constructor(options: Options = {}) {
		...
		if (
			options.concurrentTasksPerWorker !== undefined &&
		...
		if (
			options.atomics != null &&
			(typeof options.atomics !== "string" ||
				!["sync", "async", "disabled"].includes(options.atomics))
		) {
			throw new TypeError(
				"options.atomics should be a value of sync, sync or disabled.",
			);
		}
		if (
		...
		if (
			...
			(typeof options.niceIncrement !== "number" ||
				(options.niceIncrement < 0 && process.platform !== "win32"))
		) {
			throw new TypeError(
				"options.niceIncrement must be a non-negative integer on Unix systems",
			);
		}
		if (
		...
		if (
			options.closeTimeout !== undefined &&
			(typeof options.closeTimeout !== "number" || options.closeTimeout < 0)
		) {
			throw new TypeError(
				"options.closeTimeout must be a non-negative integer",
			);
		}
		if (
			options.loadBalancer !== undefined &&
			(typeof options.loadBalancer !== "function" ||
				options.loadBalancer.length < 1)
		) {
			throw new TypeError(
				"options.loadBalancer must be a function with at least two args",
			);
		}
		if (
			options.workerHistogram !== undefined &&
			typeof options.workerHistogram !== "boolean"
		) {
	...
	get histogram(): PiscinaHistogram {
		if (this.#histogram == null) {
			const piscinahistogram = {
				get runTime() {
					return this.histogram?.runTimeSummary!;
				},

				get waitTime() {
					return this.histogram?.waitTimeSummary!;
				},
				resetRunTime() {
	...
	get utilization(): number {
		...
		const totalMeanRuntime =
			(this.#pool.histogram.runTimeSummary.mean / 1000) * count;
