...
import {
	...
	BreadcrumbList,
	BreadcrumbSeparator,
} from "@react-client/features/json4u/components/ui/breadcrumb";
import {
	LeftTruncate,
	RightTruncate,
} from "@react-client/features/json4u/components/ui/truncate";
...
export default function StatusBar() {
	return (
		<div
			...
			className="w-full min-h-fit h-statusbar px-4 py-1 flex text-xs gap-4"
		>
			<JsonPath />
			...
			<BreadcrumbList className="flex-nowrap whitespace-nowrap overflow-x-auto">
				{jsonPath.map((k, i) => (
					<Fragment key={`${i}-${k}`}>
						<BreadcrumbItem
							title={k}
							className="inline-block cursor-pointer max-w-max-key truncate"
						>
							<BreadcrumbLink
								onClick={() => {
