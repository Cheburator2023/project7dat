...
declare abstract class CompositePool<
	S extends Swimlane = Swimlane,
	P extends Phase = Phase,
> extends dia.Element {
	constructor(
		attributes?: CompositePool.Attributes<CompositePool.Selectors>,
		opt?: dia.Graph.Options,
	);
	isHorizontal(): boolean;
	getMinimalXRange(): [number, number] | null;
	getMinimalYRange(): [number, number] | null;
	...
	getElementsInOrthogonalRange(
		orthogonalStartCoord: number,
		orthogonalEndCoord: number,
		options?: CompositePool.InRangeOptions,
	): dia.Element[];
	getMinimalWidth(): number;
	getMinimalHeight(): number;
	findPhaseFromOrthogonalCoord(orthogonalCoord: number): P | null;
	changeSize(
		direction: dia.OrthogonalDirection,
		dim: number | null,
	...
	protected fixSwimlanesDimensionToFitPhases(
		options?: dia.ModelSetOptions,
	): void;
	...
	protected afterSwimlaneResizeBottom(
		swimlane: S,
		options?: CompositePool.AfterSwimlaneResizeOptions<S>,
	): void;
	protected afterSwimlaneResizeTop(
		swimlane: S,
		options?: CompositePool.AfterSwimlaneResizeOptions<S>,
	): void;
	protected afterSwimlaneResizeRight(
		swimlane: S,
		options?: CompositePool.AfterSwimlaneResizeOptions<S>,
	): void;
	protected afterSwimlaneResizeLeft(
		swimlane: S,
		options?: CompositePool.AfterSwimlaneResizeOptions<S>,
	): void;
	protected afterPhaseResizeBottom(
		phase: P,
		options?: CompositePool.AfterPhaseResizeOptions<P>,
	): void;
	protected afterPhaseResizeTop(
		phase: P,
		options?: CompositePool.AfterPhaseResizeOptions<P>,
	): void;
	protected afterPhaseResizeRight(
		phase: P,
		options?: CompositePool.AfterPhaseResizeOptions<P>,
	): void;
	protected afterPhaseResizeLeft(
		phase: P,
		options?: CompositePool.AfterPhaseResizeOptions<P>,
	): void;
	...
	protected resizePhaseOrthogonalStart(
		phases: P[],
		index: number,
		diffOrthogonalDim: number,
	): void;
	protected resizePhaseOrthogonalEnd(
		phases: P[],
		index: number,
		diffOrthogonalDim: number,
	): void;
...
declare abstract class Phase extends dia.Element {
	constructor(
		attributes?: Phase.Attributes<Phase.Selectors>,
		opt?: dia.Graph.Options,
	);
...
declare abstract class SelectionRegion<
	G,
	O extends SelectionRegion.Options<G> = SelectionRegion.Options<G>,
> extends mvc.View<undefined, SVGElement> {
	paper: dia.Paper;
	options: O;
...
declare abstract class Swimlane extends dia.Element {
	constructor(
		attributes?: Swimlane.Attributes<Swimlane.Selectors>,
		opt?: dia.Graph.Options,
	);
	abstract isHorizontal(): boolean;
	getHeaderSide(): dia.OrthogonalDirection;
...
declare class BorderedRecord extends Record$1 {
	constructor(
		attributes?: Record$1.Attributes<BorderedRecord.Selectors>,
		opt?: dia.Graph.Options,
	);
}
declare class Choreography extends dia.Element {
	...
	protected buildParticipantMarkup(
		id: number,
		label: string,
		initiating: boolean,
	): dia.MarkupNodeJSON;
...
declare class CommandManager extends mvc.Model<CommandManager.Options> {
	...
	protected reduceOptions(
		opt: CommandManager.EventOptions,
	): Partial<CommandManager.EventOptions>;
	protected exportBatchCommands(
		commands?: CommandManager.Commands,
	): CommandManager.BatchCommand[];
	static sortBatchCommand(
		batchCommand: CommandManager.BatchCommand,
	): CommandManager.BatchCommand;
	static filterBatchCommand(
		batchCommand: CommandManager.BatchCommand,
	): CommandManager.BatchCommand;
	static squashCommands(
		commands: CommandManager.Commands,
		n?: number,
...
declare class Distance extends dia.Link {
	...
	protected getDistanceText(
		view: dia.LinkView,
		opt?: Distance.LabelFormatOptions,
	): string;
}
declare class Event_2 extends dia.Element {
	constructor(
		attributes?: Event_2.Attributes<Event_2.Selectors>,
		opt?: dia.Graph.Options,
	);
...
declare class ForceDirected extends mvc.Model {
	...
	protected getCellLayoutAttribute(
		cell: dia.Cell,
	): ForceDirected.CellAttributes;
	...
	protected calculateRepulsiveForceOnNode(
		u: ForceDirected.ElementData,
		quadtree: Quadtree,
	): {
		x: number;
		y: number;
	...
	protected calculateRadialForceOnNode(
		u: ForceDirected.ElementData,
		quadtree: Quadtree,
	): {
		x: number;
	...
	changeElementData(
		id: dia.Cell.ID,
		data: Partial<ForceDirected.ElementData>,
	): void;
...
declare class FreeTransform extends mvc.View<undefined> {
	...
	protected toValidResizeDirection(
		direction: FreeTransform.Directions,
	): dia.DiagonalDirection;
...
declare class HTMLSelectionFrameList extends OverlaySelectionFrameList<
	HTMLSelectionFrameList.Options,
	HTMLSelectionFrameList.Item
> {
...
declare class HeaderedHorizontalPool extends HorizontalPool {
	constructor(
		attributes?: HeaderedCompositePool.Attributes<HeaderedCompositePool.Selectors>,
		opt?: dia.Graph.Options,
	);
	getHeaderSide(): dia.OrthogonalDirection;
	getHeaderSize(): number;
...
declare class HeaderedRecord extends Record$1 {
	constructor(
		attributes?: Record$1.Attributes<HeaderedRecord.Selectors>,
		opt?: dia.Graph.Options,
	);
}
...
declare class HorizontalPool extends CompositePool<
	HorizontalSwimlane,
	VerticalPhase
> {
	constructor(
...
declare class Inspector extends mvc.View<undefined> {
	...
	static create(
		container: mvc.$HTMLElement,
		opt?: Inspector.CreateOptions,
	): Inspector;
	static close(): void;
...
declare class Keyboard {
	...
	on(
		evt: string | object,
		callback?: ((evt: dia.Event) => void) | any,
		context?: any,
	): this;
	off(
		evt: string | object,
...
declare class OverlaySelectionFrameList<
	O extends OverlaySelectionFrameList.Options,
	I = OverlaySelectionFrameList.Item<SVGElement | HTMLElement>,
> extends SelectionFrameList<O> {
...
declare class PaperScroller extends mvc.View<undefined> {
	...
	scrollToElement(
		element: dia.Element,
		opt?: PaperScroller.ScrollOptions,
	): void;
	zoom(): number;
	...
	transitionToRect(
		rect: dia.BBox,
		opt?: PaperScroller.TransitionToRectOptions,
	): this;
	...
	scrollWhileDragging(
		...
		x: number,
		y: number,
		opt?: PaperScroller.ScrollWhileDraggingOptions,
	): void;
...
declare class PolygonalSelectionRegion extends SelectionRegion<
	PolygonalSelectionRegion.Polygon,
	PolygonalSelectionRegion.Options
> {
...
declare class RadioGroup extends mvc.View<undefined> {
	...
	protected renderOption(
		option: RadioGroup.RadioGroupOption,
		index: number,
	): void;
	protected onOptionClick(evt: dia.Event): void;
}
declare class RangeSelectionRegion extends SelectionRegion<
	RangeSelectionRegion.Range,
	RangeSelectionRegion.Options
> {
	protected points: Array<g.Point>;
	preinitialize(): void;
	...
	]: RangeSelectionRegion.Range): RangeSelectionRegion.Range;
}
declare class Record$1 extends dia.Element {
	constructor(
		attributes?: Record$1.Attributes<Record$1.Selectors>,
		opt?: dia.Graph.Options,
	);
	protected metrics: any;
	item(itemId: Record$1.ItemId): Record$1.Item | null;
	item(
		itemId: Record$1.ItemId,
		value: Record$1.Item,
		opt?: dia.Cell.Options,
	): this;
	...
	addItemAtIndex(
		id: Record$1.ItemId | Record$1.GroupId,
		index: number,
		item: Record$1.Item,
		opt?: dia.Cell.Options,
	): this;
	...
	protected getGroupSelector(
		selector: string,
		...ids: Array<Record$1.ItemId | Record$1.GroupId>
	): string[];
	protected getItemLabelMarkup(
		item: Record$1.Item,
		x: number,
		y: number,
		groupId: Record$1.GroupId,
	): dia.MarkupNodeJSON;
	protected getItemBodyMarkup(
		item: Record$1.Item,
		x: number,
		y: number,
		groupId: Record$1.GroupId,
	...
	protected getIconMarkup(
		item: Record$1.Item,
		x: number,
		y: number,
		groupId: Record$1.GroupId,
	): dia.MarkupNodeJSON;
	protected getButtonMarkup(
		item: Record$1.Item,
		x: number,
		y: number,
		groupId: Record$1.GroupId,
	): dia.MarkupNodeJSON;
	protected getButtonPathData(
		x: number,
		y: number,
		r: number,
		collapsed: boolean,
	...
	protected getItemCacheAttribute(
		itemId: Record$1.ItemId,
		attribute: string,
	): any;
}
...
declare class RectangularSelectionRegion extends SelectionRegion<
	RectangularSelectionRegion.Rect,
	RectangularSelectionRegion.Options
> {
	protected points: Array<g.Point>;
	preinitialize(): void;
	...
	normalizeGeometry(
		rect: RectangularSelectionRegion.Rect,
	): RectangularSelectionRegion.Rect;
}
declare class SVGSelectionFrameList extends OverlaySelectionFrameList<
	SVGSelectionFrameList.Options,
	SVGSelectionFrameList.Item
> {
...
declare class SearchGraph extends dia.Graph {
	...
	protected queryQuadTreeRect(
		rect: dia.BBox,
		strict?: boolean,
	): SearchGraph.Object[];
	protected queryQuadTreePoint(
		point: dia.Point,
		strict?: boolean,
	): SearchGraph.Object[];
	protected getInitialQuadTreeBoundary(): dia.BBox;
	protected computeInitialQuadTreeBoundary(
		width: number,
		height?: number,
	): dia.BBox;
	...
	findElementsInArea(
		area: dia.BBox,
		options?: dia.Graph.FindInAreaOptions,
	): dia.Element[];
	findLinksInArea(
		area: dia.BBox,
		options?: dia.Graph.FindInAreaOptions,
	...
	findElementsAtPoint(
		point: dia.Point,
		options?: dia.Graph.FindAtPointOptions,
	): dia.Element[];
	findLinksAtPoint(
		point: dia.Point,
		options?: dia.Graph.FindAtPointOptions,
	...
	findElementsUnderElement(
		element: dia.Element,
		options?: dia.Graph.FindUnderElementOptions,
	): dia.Element[];
	...
	protected findUnderElement<T extends dia.Cell>(
		element: dia.Element,
		findInArea: (area: dia.BBox, opt: dia.Graph.FindInAreaOptions) => T[],
		findAtPoint: (point: dia.Point, opt: dia.Graph.FindAtPointOptions) => T[],
		options?: dia.Graph.FindUnderElementOptions,
	): T[];
}
...
declare class Selection extends mvc.View<dia.Cell> {
	...
	protected startSelectionInteraction(
		evt: dia.Event,
...
declare class SelectionWrapper<E extends Element = Element> extends mvc.View<
	undefined,
	E
> {
...
declare class Snaplines extends mvc.View<undefined> {
	...
	protected initializeSnapWhileMoving(
...
declare class StackLayoutView extends mvc.View<
	StackLayoutView.StackLayoutModel,
	SVGElement
> {
...
declare class Stencil extends mvc.View<undefined> {
	...
	filter(
		keyword: string,
		matchCell?: Stencil.MatchCellMap | Stencil.MatchCellCallback,
	): void;
	setCellCursor(cursor: string): void;
	protected positionCell(
		cell: dia.Cell,
		x: number,
		y: number,
		opt?: dia.Cell.Options,
	): void;
	protected preparePaperForDragging(
		cell: dia.Cell,
		clientX: number,
		clientY: number,
	): void;
	...
	protected notifyDragEnd(
		cloneView: dia.CellView,
		evt: dia.Event,
		cloneArea: g.Rect,
...
declare class SwimlaneTransform extends dia.ToolView {
	...
	protected onHandleChangeStart(
		selectedHandle: SwimlaneTransform.Handle,
		evt: dia.Event,
	): void;
...
declare class Toolbar extends mvc.View<undefined> {
	...
	getWidgetByName<T extends keyof widgets.WidgetMap>(
		name: string,
	): widgets.WidgetMap[T];
...
declare class TreeLayout extends mvc.Model {
	...
	updateDirections(
		branch: dia.Element,
		rule: TreeLayout.FromToDirections,
		opt?: dia.Cell.Options,
	): void;
	reconnectElement(
		child: dia.Element,
		parent: dia.Element,
		opt?: TreeLayout.ReconnectElementOptions,
	): boolean;
	changeSiblingRank(
		el: dia.Element,
...
declare class VerticalPool extends CompositePool<
	VerticalSwimlane,
	HorizontalPhase
> {
	constructor(
...
declare const Dijkstra: (
	...
	weight: (aNode: any, bNode: any) => number,
) => any;
...
declare function toCellsArray(
	xmlString: string,
	makeElement: (opt: ElementOptions, xmlNode: Node) => dia.Element,
	makeLink: (opt: LinkOptions, xmlEdge: Node) => dia.Link,
): Array<dia.Cell>;
...
declare function toJPEG(
	paper: dia.Paper,
	callback: (dataURL: string, error?: Error) => void,
	opt?: RasterExportOptions,
): void;
declare function toPNG(
	paper: dia.Paper,
	callback: (dataURL: string, error?: Error) => void,
	opt?: RasterExportOptions,
): void;
declare function toSVG(
	paper: dia.Paper,
...
declare interface ConstructTreeConfig {
	children?:
		| string
		| ((node: ConstructTreeNode | any) => ConstructTreeNode[] | any);
	makeElement: (
		node: ConstructTreeNode,
		parentElement: dia.Element | null,
		index: number | null,
	) => dia.Element;
...
declare namespace GridLayout {
	type SetAttributesCallback = (
		element: dia.Element,
		attributes: dia.Element,
		opt: Options,
	) => void;
	type GridMetrics = {
		rowHeights: number[];
	...
	export function layout(
		graphOrCells: dia.Graph | Array<dia.Cell>,
		opt?: Options,
	): GridMetrics;
}
...
declare namespace HTMLSelectionFrameList {
	type ItemStyle =
		...
		| ((
				cell: dia.Cell,
				frameList: HTMLSelectionFrameList,
		  ) => Partial<CSSStyleDeclaration>);
...
declare namespace Halo {
	export interface Options extends mvc.ViewOptions<undefined> {
		...
		boxContent?:
			| boolean
			| mvc.$HTMLElement
			| ((cellView: dia.CellView, boxElement: HTMLElement) => mvc.$HTMLElement);
		groups?: {
		...
		bbox?:
			| dia.Point
			| dia.BBox
			| ((cellView: dia.CellView) => dia.Point | dia.BBox);
	...
	type MakeElementCallback = (
		ctx: MakeElementContext,
		evt: dia.Event,
		halo: Halo,
	) => dia.Element;
...
declare namespace HighlighterSelectionFrameList {
	...
	type HighlighterOptions =
		...
		| ((
				cell: dia.Cell,
				frameList: HighlighterSelectionFrameList,
		  ) => dia.HighlighterView.Options);
	interface Options {
...
declare namespace Matrix {
	interface Selectors
		extends shapes.SVGRectSelector,
			shapes.SVGTextSelector,
			shapes.SVGPathSelector {
...
declare namespace PaperScroller {
	...
	interface Options extends mvc.ViewOptions<undefined> {
		...
		baseHeight?: number;
		contentOptions?:
			| dia.Paper.FitToContentOptions
			| ((paperScroller: PaperScroller) => dia.Paper.FitToContentOptions);
		cursor?: string;
...
declare namespace Popup {
	...
	export interface Options extends ContextToolbar.Options {
		...
		content?:
			| mvc.$HTMLElement
			| ((el: SVGElement) => undefined | mvc.$HTMLElement);
...
declare namespace SVGSelectionFrameList {
	type ItemAttributes =
		| attributes.NativeSVGAttributes
		| ((
				cell: dia.Cell,
				frameList: SVGSelectionFrameList,
		  ) => attributes.NativeSVGAttributes);
	interface Options extends OverlaySelectionFrameList.Options {
...
declare namespace Selection {
	interface Options extends mvc.ViewOptions<undefined> {
		...
		boxContent?:
			| boolean
			| mvc.$HTMLElement
			| ((boxElement: HTMLElement) => mvc.$HTMLElement);
		handles?: Array<Handle>;
		useModelGeometry?: boolean;
...
declare namespace Snaplines {
	...
	export interface Options extends mvc.ViewOptions<undefined> {
		...
		additionalSnapPoints?: (
			this: Snaplines,
			elementView: dia.ElementView,
			options: AdditionalSnapPointsOptions,
		) => Array<dia.Point>;
	}
}
...
declare namespace StackLayoutView {
	...
	export type PreviewCallback = (
		options: PreviewCallbackOptions,
		view: StackLayoutView,
	) => SVGElement;
	export type ValidateMovingCallback = (
		options: ValidateCallbackOptions,
		view: StackLayoutView,
	) => boolean;
	export type ModifyInsertElementIndexCallback = (
		options: ModifyInsertElementIndexOptions,
		point: g.Point,
		view: StackLayoutView,
	) => number;
	export type InsertElementCallback = (
		options: InsertElementOptions,
		view: StackLayoutView,
	) => void;
	export type CanInteractCallback = (
		elementView: dia.ElementView,
		view: StackLayoutView,
		evt: dia.Event,
	) => boolean;
	export interface StackLayoutModelOptions
		extends StackLayout.StackLayoutOptions {
		stackIndexAttributeName?: string;
		stackElementIndexAttributeName?: string;
	}
	export interface StackLayoutViewOptions
		extends mvc.ViewOptions<StackLayoutModel, SVGElement> {
...
declare namespace SwimlaneTransform {
	...
	export interface Options extends dia.ToolView.Options {
		...
		minSizeConstraints?: (
			model: Pool,
			laneId: string,
			handleSide: dia.OrthogonalDirection,
		) => Array<dia.Point>;
		maxSizeConstraints?: (
			model: Pool,
			laneId: string,
...
declare namespace TextEditor {
	...
	interface EventMap {
		...
		"text:change": (
			text: string,
			prevText: string,
			annotation: Array<Vectorizer.TextAnnotation>,
			selection: Selection,
	...
	export interface Options extends mvc.ViewOptions<undefined> {
		...
		content?:
			...
			| ((
					...
					node: Node,
					tooltip: Tooltip,
			  ) => mvc.$Element | false | null | undefined);
...
declare namespace TreeLayout {
	...
	type UpdatePositionCallback = (
		element: dia.Element,
		position: dia.Point,
		opt: UpdateOptions,
		treeLayout: TreeLayout,
	) => void;
	type UpdateVerticesCallback = (
		link: dia.Link,
		vertices: Array<dia.Point>,
		opt: UpdateOptions,
		treeLayout: TreeLayout,
	) => void;
	type UpdateSiblingRankCallback = (
		element: dia.Element,
		siblingRank: SiblingRank,
		opt: UpdateOptions,
		treeLayout: TreeLayout,
	) => void;
	type UpdateAttributesCallback = (
...
declare namespace bpmn2 {
	export {
		...
		Gateway,
		Conversation,
		type ConversationLinkSelectors,
		ConversationLink,
		Annotation,
		type AnnotationLinkSelectors,
		AnnotationLink,
		DataObject,
		DataAssociation,
		DataStore,
...
declare namespace gexf {
	export { toCellsArray, type ElementOptions, type LinkOptions };
}
...
declare module "@joint/core" {
	...
	export namespace graphUtils {
		export {
			...
			shortestPath,
			toAdjacencyList,
			type ConstructTreeNode,
			type ConstructTreeConfig,
	...
	export namespace alg {
		export { Dijkstra, PriorityQueue, type PriorityQueueOptions };
	}
