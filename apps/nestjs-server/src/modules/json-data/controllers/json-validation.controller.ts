...
export class JsonValidationController {
    ...
    @ApiOperation({
        ...
        description: "Проверяет JSON данные на корректность и выявляет потенциальные проблемы"
    })
    @ApiBody({
    ...
    async validateJson(
        ...
        const structureValidation = this.jsonMappingService.validateJsonStructure(data);
        ...
        if (data.entities && Array.isArray(data.entities)) {
            for (const entity of data.entities) {
                if (entity.type) {
                    const isValidType = await this.entityTypeService.validateEntityType(entity.type);
                    if (!isValidType) {
                        warnings.push(`Неизвестный тип сущности: "${entity.type}" для "${entity.id}"`);
                    }
                }
            }
        }
            ...
            for (const entity of data.entities) {
                if (entity.attrSeq && Array.isArray(entity.attrSeq)) {
                    for (const attr of entity.attrSeq) {
                        if (attr.type) {
                            const isValidType = await this.attributeTypeService.validateAttributeType(attr.type);
                            if (!isValidType) {
                                warnings.push(`Неизвестный тип атрибута: "${attr.type}" для атрибута "${attr.name}" в сущности "${entity.id}"`);
                            }
                        }
                    }
                }
        ...
        const modifiedEntities = (data.entities || []).filter((entity: any) => entity.modified);
        if (modifiedEntities.length > 0) {
            const processId = await this.jsonMappingService.getProcessIdFromData(data);
    ...
    async checkDependencies(
        ...
        return await this.dependencyCheckService.checkMartUsage(entityFullNames, currentProcessId);
    }
}