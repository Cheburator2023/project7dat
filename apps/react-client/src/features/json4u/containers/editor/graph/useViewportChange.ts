...
import {
	clearHighlight,
	highlightElement,
} from "@react-client/features/json4u/lib/graph/utils";
import { refreshInterval } from "@react-client/features/json4u/lib/graph/virtual";
...
export function useRevealNode(
	...
	const { isNeedReveal, revealPosition } = useStatusStore(
		useShallow((state) => ({
			isNeedReveal:
				state.viewMode === ViewMode.Graph && state.isNeedReveal("graph"),
	...
	const isMeasured =
		waitToMeasure.length &&
		waitToMeasure.every((id) => nodesMap[id]?.measured?.width);
	...
	useEffect(() => {
		...
		if (isNeedReveal && isMeasured && treeNodeId) {
			(async () => {
				...
				if (isKV) {
					const kvEl = document.querySelector(
						`.graph-kv[data-tree-id="${treeNodeId}"]`,
					);
					el = kvEl?.querySelector(
						`.${type === "value" ? "graph-v" : "graph-k"}`,
					) as HTMLDivElement;
				} else {
					el = document.querySelector(
						`.graph-node[data-tree-id="${treeNodeId}"]`,
					) as HTMLDivElement;
				}
