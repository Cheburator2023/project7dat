	...
	stringify(value: any, options: JsonStringifyOptions = {}): string {
		...
		try {
			...
			if (schema) {
				...
				const fastStringify = this.stringifyCache.get(cacheKey);
			...
			if (safe) {
				throw new Error(
					`JSON stringification failed: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
			throw error;
		}
...
const devalueSerialize = (value: any): string => {
	try {
		...
		throw new Error(
			`Devalue serialization failed: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
	}
};
...
const devalueDeserialize = <T = any>(text: string): T => {
	try {
		...
		throw new Error(
			`Devalue deserialization failed: ${error instanceof Error ? error.message : "Unknown error"}`,
		);
	}
};
