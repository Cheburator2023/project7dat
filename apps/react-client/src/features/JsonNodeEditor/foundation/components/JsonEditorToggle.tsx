...
const _JsonEditorToggle = () => {
	const [isJsonEditorVisible, toggleJsonEditor] = useJsonEditorViewStore(
		(state) => [state.isJsonEditorVisible, state.toggleJsonEditor],
	);
	...
	return (
		<CircleTransparentButton onClick={toggleJsonEditor}>
			<Icon
				icon={
					isJsonEditorVisible ? "left-arrow-with-bar" : "right-arrow-with-bar"
				}
				size={24}
