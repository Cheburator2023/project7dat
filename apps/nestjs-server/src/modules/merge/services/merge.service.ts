...
export class MergeService {
	...
	constructor(
		...
		private readonly commitRepository: Repository<S2tCommitEntity>,
		private readonly jsonExportService: JsonExportService,
		private readonly jsonImportService: JsonImportService,
		private readonly s2tCommitService: S2tCommitStoreService,
	...
	async applyMerge(commitId: string): Promise<ApplyMergeResponseDto> {
		...
		const commitType =
			(commit.payload as any)?.desc?.commit_type || commit.type;
		...
		const _recursionMerged = this.structureValidator.checkForRecursion(
