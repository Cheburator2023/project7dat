import { escape as escapeStr, filter } from "lodash-es";

export function h(tag = "", ...children: (H | string)[]) {
...
export class H {
	...
	constructor(tag = "", ...children: (H | string)[]) {
		this.tag = tag;
		this.attrClass = [];
	...
	toString(): string {
		const childrenStr = this.children
			.map((child) =>
				typeof child === "string" ? escapeStr(child) : child.toString(),
			)
			.join("");
		...
		if (this.tag === "") {
			...
			this.attrClass.length &&
				attrs.push(`class="${this.attrClass.join(" ")}"`);
	...
	toDom(): HTMLElement | DocumentFragment {
		const el = this.tag
			? (() => {
					...
					if (this.attrClass.length)
						el.setAttribute("class", this.attrClass.join(" "));
		...
		el.append(
			...this.children.map((child) =>
				typeof child === "string" ? child : child.toDom(),
			),
		);
		return el;
	}
