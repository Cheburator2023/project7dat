...
function genDom(tree: Tree, node: Node, addExpander?: boolean): H {
	...
	if (isIterable(node)) {
		if (hasChildren(node)) {
			...
			return el.child(
				(node.type === "array" ? genArrayDom : genObjectDom)(tree, node).id(id),
			);
		} else {
...
function genArrayDom(tree: Tree, node: Node) {
	...
	const rowForHeaders = headers.length
		? h("tr", indexHeader)
				.addChildren(
					headers.map((key) =>
						genTableHeader(tree, node, key, key2ExpanderId[key]),
					),
				)
				.class("sticky-scroll")
		: "";
	...
	const rows = tree.mapChildren(node, (child, i) => {
		const indexCell = existsLeafNode
			? h("td", h("span", i).class("tbl-no")).class("tbl-index")
			: "";
		let valueCells: H[] = [];

		if (hasChildren(child)) {
			// generate each column for the row
			valueCells = headers.map((key) =>
				h(
					"td",
					genDom(tree, tree.getChild(child, key)!, !!key2ExpanderId[key]),
				),
			);
		} else {
...
function genTableHeader(
	...
	const title = path
		.map(({ nodeType, key }, i) => {
			...
			const prefix =
				i < path.length - 1 ? nodeTypeToPrefixMap[nodeType!] + " " : "";
			return `${prefix}${key.replaceAll('"', "&quot;")}`;
		})
		.join("\n");
	const keyDom = h("span", key || '""').class(
		key ? "text-hl-key" : "text-hl-empty",
	);

	return h(
		"th",
		h("div", keyDom, genExpander(expanderId)).class("tbl-key"),
	).title(title);
}
...
function genExpander(expanderId?: string) {
	...
	return h("div")
		.id(expanderId)
		.class("tbl-expander", "codicon", "codicon-folding-expanded");
}
...
export function genKeyAndTypeList(
	tree: Tree,
	id: string,
	...keys: string[]
): KeyWithType[] {
