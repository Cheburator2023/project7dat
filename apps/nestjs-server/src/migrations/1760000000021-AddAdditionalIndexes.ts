...
export class AddAdditionalIndexes1760000000021 implements MigrationInterface {
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`DROP INDEX IF EXISTS idx_entity_map_process_entity`,
		);
		await queryRunner.query(
			`DROP INDEX IF EXISTS idx_attribute_map_source_attribute`,
		);
		await queryRunner.query(
			`DROP INDEX IF EXISTS idx_entity_attribute_map_deptype`,
		);
		await queryRunner.query(
			`DROP INDEX IF EXISTS idx_failed_mappings_entity_name`,
		);
