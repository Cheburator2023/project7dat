import type {
	...
	DiffType,
	Range,
} from "@react-client/features/json4u/lib/compare";
import { newRange } from "@react-client/features/json4u/lib/compare";
...
export class Comparer {
	...
	async compare() {
		const isTextCompare =
			this.enableTextCompare() ||
			!(this.main.isTreeValid() && this.secondary.isTreeValid());
	...
	applyDecorations({
		...
		this.rightDecorations =
			this.monacoEditor("secondary").createDecorationsCollection(right);
	}
...
function genHighlightDecorations(diffPairs: DiffPair[]): {
	...
	const compare = (a: Decoration, b: Decoration) =>
		a.range.startLineNumber - b.range.startLineNumber;
	const left = leftDecorations.concat(
		mergeDecorations(leftHunks.sort(compare)),
	);
	const right = rightDecorations.concat(
		mergeDecorations(rightHunks.sort(compare)),
	);
	return { left, right };
}
...
function mergeDecorations(decorations: Decoration[]): Decoration[] {
	...
	for (const decoration of decorations.slice(1)) {
		...
		if (startLineNumber <= prev.range.endLineNumber) {
			prev.range.endLineNumber = Math.max(
				endLineNumber,
				prev.range.endLineNumber,
			);
		} else {
...
function genDecorations(diff: Diff | undefined): {
	...
	const inlines =
		diff.inlineDiffs?.map((d) => newDecoration(d.range!, true, type)) ?? [];
	return { hunk, inlines };
}
