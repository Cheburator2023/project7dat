...
export class Tree implements TreeObject {
	...
	static assign<A extends Tree | TreeObject, B extends Tree | TreeObject>(
		a: A,
		b: B,
	) {
	...
	mapChildren<T>(
		node: Node,
		fn: (child: Node, key: string, index: number) => T,
	): T[] {
		return getChildrenKeys(node).map((key, i) =>
			fn(this.getChild(node, key)!, key, i),
		);
	}
	...
	findNodeAtOffset(
		offset: number,
	): { node: Node; type: "node" | "key" | "value" } | undefined {
		if (!this.valid()) {
			return undefined;
		}
		...
		const inBound = (node: Node) => {
			...
			return (
				...
				offset <= node.boundOffset + node.boundLength
			);
		};
		...
		if (this.isGraphNode(node)) {
			...
			return {
				node,
				type:
					node.offset < offset && offset <= node.offset + node.length
						? "value"
						: "key",
			};
		}
	...
	stringifyNode(
		...
		indentLevel = 0,
		offset = 0, // the starting offset of the current node in the stringify text.
		boundOffset = 0, // the starting offset of the current node's bound in the stringify text.
		genTabs: ReturnType<typeof getGenTabsFn> = getGenTabsFn(
			options.tabWidth || 2,
		),
	): string {
		if (!isIterable(node)) {
