...
import type { RevealPosition } from "@react-client/features/json4u/lib/graph/types";
import type { ParseOptions } from "@react-client/features/json4u/lib/parser";
	...
	persist(
		(set, get) => ({
			...
			setRevealPosition(pos: Partial<RevealPosition>) {
				...
				const needUpdate = !(
					oldPos.type === pos.type && oldPos.treeNodeId === pos.treeNodeId
				);
			...
			isNeedReveal(scene: "editor" | "graph") {
				...
				if (scene === "editor") {
					return enableSyncScroll
						? from !== "editor"
						: includes(["statusBar", "graphAll"], from);
				} else if (scene === "graph") {
					return enableSyncScroll
						? from !== "graphOthers"
						: includes(["statusBar", "search", "graphAll"], from);
				}
