import { type DiffPair, classify, newDiff } from "./diff";
...
class HistogramDiffer {
	...
	longestSubstring(
		...
		while (bIdx < bEnd) {
			...
			for (const as of histogram.get(lineB)) {
				...
				while (
					r.validStart(aStart, bStart) &&
					this.eq(r.aStart - 1, r.bStart - 1)
				) {
					r.aStart--;
					r.bStart--;
					if (r.matchScore > 1) {
				...
				if (
					(bestMatch && bestMatch.length() < r.length()) ||
					r.matchScore < bestMatchScore
				) {
					bestMatch = r;
	...
	fineSolveRegion(regions: MatchRegion[]): MatchRegion[] {
		...
		for (let i = 0; i <= regions.length; i++) {
			...
			const aa = this.aa.slice(
				aOffset,
				region ? region.aStart : this.aa.length,
			);
			const bb = this.bb.slice(
				bOffset,
				region ? region.bStart : this.bb.length,
			);
	...
	solve(): DiffPair[] {
		...
		const lineLen = (line: string) =>
			line === "" ? 0 : Math.max(1, line.length - 1);

		let apos = 0;
		let bpos = 0;
		let prevRegion = new MatchRegion(0, 0, 0, 0, 0);
		// 找到公共行
		const regions = this.fineSolveRegion(
			this.solveRange(0, this.aa.length, 0, this.bb.length),
		);
		...
		for (let i = 0; i <= regions.length; i++) {
			...
			if (aaStr.length > 0 && bbStr.length > 0) {
				const { left, right } = classify(
					myersDiff(aaStr, bbStr, { maxEditLength: 100 }),
				);
