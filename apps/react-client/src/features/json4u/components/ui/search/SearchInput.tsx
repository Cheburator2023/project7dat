...
import {
	...
	CommandItem,
	CommandList,
} from "@react-client/features/json4u/components/ui/command";
...
export default function SearchInput<T extends { id: string }>({
	...
	const height =
		items.length > 0
			? Math.min(
					items.length * itemHeight + 2 * commandGroupPaddingY,
					searchMenuHeight,
				)
		...
		>
			<CommandInput
				...
				onBlur={(ev) => {
					...
					if (
						commandRef.current &&
						!commandRef.current.contains(ev.relatedTarget as Node)
					) {
						setOpen(false);
					}
				}}
			...
			>
				{displayShortcut && bindShortcut && (
					<CommandInputShortcut>{`⌘ ${bindShortcut}`}</CommandInputShortcut>
				)}
			</CommandInput>
			<CommandList>
				{items.length === 0 && (
