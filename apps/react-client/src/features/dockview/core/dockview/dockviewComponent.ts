	...
	implements IDockviewComponent
{
	...
	constructor(container: HTMLElement, options: DockviewComponentOptions) {
		...
		this._rootDropTarget = new Droptarget(this.element, {
			...
			overlayModel: options.rootOverlayModel ?? DEFAULT_ROOT_OVERLAY_MODEL,
	...
	addFloatingGroup(
		...
		overlay.setupDrag(<HTMLElement>el, {
			inDragMode:
				typeof options?.inDragMode === "boolean" ? options.inDragMode : false,
		});
	...
	fromJSON(
		...
		if (!Array.isArray(grid.root.data)) {
			throw new Error("dockview: root.data is not an array");
		}
		...
		try {
			...
			const createGroupFromSerializedState = (data: GroupPanelViewState) => {
				const { id, locked, hideHeader, headerPosition, views, activeView } =
					data;
		...
		if (referenceGroup) {
			...
			if (options.floating) {
				...
				const floatingGroupOptions =
					typeof options.floating === "object" && options.floating !== null
						? options.floating
						: {};
				...
				const group = this.createGroupAtLocation(
					relativeLocation,
					this.orientationAtLocation(relativeLocation) === Orientation.VERTICAL
						? initial?.height
						: initial?.width,
				);
				panel = this.createPanel(options, group);
		...
		if (!destinationTarget || destinationTarget === "center") {
			...
			this.movingLock(() =>
				destinationGroup.model.openPanel(removedPanel, {
					...
					skipSetActive:
						(options.skipSetActive ?? false) && !isDestinationGroupEmpty,
					skipSetGroupActive: true,
				}),
			);
			if (!options.skipSetActive) {
	...
	private updateDropTargetModel(options: Partial<DockviewComponentOptions>) {
		if ("dndEdges" in options) {
			this._rootDropTarget.disabled =
				typeof options.dndEdges === "boolean" && options.dndEdges === false;
