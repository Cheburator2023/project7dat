...
export class CacheService {
    ...
    async setCachedExportAll(data: any, ttl?: number): Promise<void> {
        await this.executeCacheOperation(
            ...
            this.allCacheKey,
            () => this.cacheManager.set(this.allCacheKey, data, ttl || this.getDefaultTtl())
        );
    }
    ...
    getMetrics(): CacheMetrics {
        ...
        this.metrics.hitRatio = totalOperations > 0
            ? (this.metrics.hits / totalOperations) * 100
            : 0;
     ...
     */
    private updateMetrics(type: CacheOperationType, isSuccess: boolean, isError = false): void {
        if (isError) {
            this.metrics.errors++;
        } else if (isSuccess) {
            switch (type) {
