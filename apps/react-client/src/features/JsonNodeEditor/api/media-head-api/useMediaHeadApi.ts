...
import type { MediaHead } from "./media-head.types";

const queryKeys = {
	all: ["mediaHeads"],
	mediaHeadBySrc: (mediaSrc: UseMediaHeadApiRequest["mediaSrc"]) => [
		...queryKeys.all,
		mediaSrc,
	],
} as const;
...
export const useMediaHeadApi = (
	request: UseMediaHeadApiRequest,
): UseQueryResult<MediaHead, Error> => {
	...
	return useQuery<MediaHead, Error>({
		queryKey: queryKeys.mediaHeadBySrc(request.mediaSrc),
		queryFn: getMediaHead,
	});
};
