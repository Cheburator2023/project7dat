...
class JsonBenchmark {
	...
	async runBenchmark(): Promise<BenchmarkResult[]> {
		...
		console.log(
			`📦 Test data complexity: ${JSON.stringify(this.testData).length} chars (without circular refs)`,
		);
		console.log("\n" + "=".repeat(80));
	...
	private async benchmarkDevalue(): Promise<BenchmarkResult> {
		...
		try {
			...
			const _deserializeTime = _performance.now() - deserializeStart;
	...
	private printResults(results: BenchmarkResult[]): void {
		...
		successfulResults.forEach((result, index) => {
			...
			const speedup =
				index === 0
					? ""
					: ` (${(result.timeMs / successfulResults[0].timeMs).toFixed(2)}x slower)`;

			console.log(`${medal} ${result.library}:`);
			console.log(`   ⏱️  Total time: ${result.timeMs.toFixed(2)}ms`);
			console.log(
				`   ⚡ Avg per op: ${(result.avgTimePerOp * 1000).toFixed(3)}μs${speedup}`,
			);
		...
		if (successfulResults.length >= 2) {
			...
			console.log(
				`   🚀 ${fastest.library} is ${improvement}x faster than ${slowest.library}`,
			);

			const devalueResult = successfulResults.find(
				(r) => r.library === "devalue",
			);
			if (devalueResult) {
			...
			console.log(
				`   ⚡ Best _performance: ${(fastest.avgTimePerOp * 1000).toFixed(3)}μs per operation`,
			);
		}
