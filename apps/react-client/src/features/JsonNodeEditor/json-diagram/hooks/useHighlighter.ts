import { type Edge, useEdges } from "reactflow";
import {
	type HoveredNodeDetail,
	useNodeDetailViewStore,
} from "../../store/node-detail-view/node-detail-view.store";
import { isString } from "../../utils/json.util";

const isConnectedEdgeToHovered = (
	edge: Edge,
...
export const useHighlighter = () => {
	const hoveredNodeDetails = useNodeDetailViewStore(
		(state) => state.hoveredNodeDetails,
	);
	const edges = useEdges();

	const highlightedEdges: Edge[] = edges.filter((edge) =>
		isConnectedEdgeToHovered(edge, hoveredNodeDetails),
	);

	const isHighlightEdge = (edgeId: string) =>
		highlightedEdges.some(({ id }) => id === edgeId);

	/**
	 * If the left edge is highlighted, the node connected to left edge should also be highlighted.
	 * Therefore, check `target` value.
	 */
	const isHighlightNode = (nodeId: string) =>
		highlightedEdges.some(({ target }) => target === nodeId);
