import { getConfig } from "@react-client/features/json4u/lib/db/config";
import { parseJSON } from "@react-client/features/json4u/lib/parser";

export async function urlToJSON(
	text: string,
): Promise<{ text: string; parse: boolean }> {
	if (!text.trim()) {
...
function urlToMap(s: string) {
	...
	const u = new URL(
		isIntact ? s : `http://json4u.com${s.startsWith("/") ? s : "/" + s}`,
	);
	const m = new Map();
	const q = new Map();
