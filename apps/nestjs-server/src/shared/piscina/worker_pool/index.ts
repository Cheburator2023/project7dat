...
export class WorkerInfo extends AsynchronouslyCreatedResource {
	...
	constructor(
		{ worker, port, enableHistogram }: WorkerInfoParams,
		onMessage: ResponseCallback,
	) {
		super();
	...
	_handleResponse(message: ResponseMessage): void {
		...
		this.histogram?.record(
			PiscinaHistogramHandler.toHistogramIntegerNano(message?.time!),
		);
	...
	currentUsage(): number {
		return this.isRunningAbortableTask()
	...
	get interface(): PiscinaWorker {
		...
		return {
			...
			get histogram() {
				return worker.histogram != null
					? PiscinaHistogramHandler.createHistogramSummary(worker.histogram)
					: null;
			},
			get terminating() {
