...
import {
	...
	isObjectSeaNode,
	isPrimitiveSeaNode,
} from "../../store/json-engine/helpers/sea-node.helper";
import {
	type JsonTree,
	useJsonEngineStore,
} from "../../store/json-engine/json-engine.store";
import type { SeaNode } from "../../store/json-engine/types/sea-node.type";
...
const getNodePath = (jsonTree: JsonTree, selfNodeId: string): NodePath => {
	...
	const isRootSelfNode =
		(isObjectSeaNode(selfNode) || isArraySeaNode(selfNode)) &&
		selfNode.data.isRootNode;
	...
	const fullNodePath = fullNodePathIds
		...
		.reduce(
			(
				...
				nodeId: string,
				index: number,
				_fullNodePathIds: string[],
			) => {
				const node: SeaNode = seaNodeEntities[nodeId];

				const parentNodeId =
					index + 1 <= _fullNodePathIds.length
						? _fullNodePathIds[index + 1]
						: undefined;
				const parentNode: SeaNode | undefined =
					parentNodeId !== undefined
						? seaNodeEntities[parentNodeId]
						: undefined;

				const isParentObjectNode =
					parentNode !== undefined && isObjectSeaNode(parentNode);
				const isParentArrayNode =
					parentNode !== undefined && isArraySeaNode(parentNode);
				...
				if (isObjectSeaNode(node)) {
					if (node.data.isRootNode) {
						...
						if (isParentObjectNode) {
							const propertyK = getObjectPropertyK({
								edges,
