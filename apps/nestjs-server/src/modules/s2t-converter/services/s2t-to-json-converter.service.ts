...
export class S2tToJsonConverterService {
    ...
    private convertMart(
        ...
        for (const [targetFullName, target] of targets.entries()) {
            const mapping = this.buildMapping(targetFullName, target.system, target.rows, sources, options);
    ...
    private convertModel(
        ...
        if (targets.size === 0) {
            throw new BadRequestException('Не найден целевой входящий вектор (Target)');
        }
        ...
        targets.forEach((t, fullName) => {
            const entity = this.buildEntity(fullName, t, t.rows, true, 'input_vector');
            if (entity.attrSeq.length > 0) {
                entities.push(entity);
            }
        });
