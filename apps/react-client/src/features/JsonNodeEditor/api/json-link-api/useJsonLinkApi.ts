...
const queryKeys = {
	all: ["jsonLinks"],
	jsonLinkByUri: (httpUri: UseJsonLinkApiRequest["httpUri"]) => [
		...queryKeys.all,
		httpUri,
	],
} as const;
 ...
 */
export const useJsonLinkApi = (
	request: UseJsonLinkApiRequest,
): UseQueryResult<JsonLink | null, Error> => {
	...
	return useQuery<JsonLink | null, Error>({
		queryKey: queryKeys.jsonLinkByUri(request.httpUri),
		queryFn: getJsonLink,
	});
};
