...
const _NodeShell = ({ nodeId, nodeType, isHighlight, children }: Props) => {
	const [selectedNodeId, selectNode] = useJsonDiagramViewStore((state) => [
		...
		<div
			style={{
				backgroundColor: isSelected
					? semanticColors[theme].primary[50]
					: undefined,
			...
			onKeyUp={(e) => e.key === "Enter" && selectNode(nodeId)}
		>
			{isLocalhost && (
