...
import type { ObjectNodeData } from "../../../store/json-engine/types/sea-node.type";
...
const _ObjectNodeDetail = ({ nodeId, nodeData }: Props) => {
	const renderPropertyCards = useCallback(() => {
		const InferredLatLngMapCardProps: any[] = inferMap(nodeData.obj, nodeId);
		...
		return Object.entries(nodeData.obj).map(([propertyK, propertyV]) => {
			...
			const inferredMapCardPropsIndex: number =
				InferredLatLngMapCardProps.findIndex(
	...
	return (
		<NodeDetailList>
			{isEmptyObject(nodeData.obj) ? (
				<EmptyNodeMessage nodeType={NodeType.Object} />
			) : (
				renderPropertyCards()
			)}
		</NodeDetailList>
