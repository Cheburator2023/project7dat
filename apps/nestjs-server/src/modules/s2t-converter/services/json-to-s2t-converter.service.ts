...
export class JsonToS2tConverterService {
    ...
    private buildMart(json: any): [string[], any[][]] {
        ...
        if (!targetEntity) throw new BadRequestException('Целевая сущность не найдена');
    ...
    private buildJson(json: any): [string[], any[][]] {
        ...
        const target = json.entities?.find((e: any) => e.modified === true && e.type === 'json');
    ...
    private buildModel(json: any): [string[], any[][]] {
        ...
        const target = json.entities?.find((e: any) => e.modified === true && e.type === 'input_vector');
    ...
    private martHeaders(): string[] {
        return [
            '', 'База/Система', '', 'Схема', '', 'Таблица', 'Код атрибута', 'Краткое описание атрибута', 'Тип данных',
            '', '', '', '', '', 'commit', '', '', '', 'База/Система', '', 'Схема', '', 'Таблица', 'Код атрибута',
            'Краткое описание атрибута', '', 'Тип данных', '', '', '', '', '', '',
        ];
    }
