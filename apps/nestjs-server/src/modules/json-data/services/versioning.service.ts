...
export class VersioningService {
	...
	validateVersionCompatibility(schemaVersion: string): {
		...
		const currentMajor = Number.parseInt(
			this.currentSchemaVersion.split(".")[0],
			10,
		);
		const incomingMajor = Number.parseInt(version.split(".")[0], 10);
		const currentMinor = Number.parseInt(
			this.currentSchemaVersion.split(".")[1] || "0",
			10,
		);
	...
	handleBackwardCompatibility(data: any, schemaVersion: string): any {
		this.logger.log(
			`Обработка обратной совместимости для версии: ${schemaVersion}`,
		);
		...
		this.logger.log(
			`Обратная совместимость обработана для версии: ${schemaVersion}`,
		);
		return normalized;
	}
	...
	migrateDataToCurrentVersion(data: any, fromVersion: string): any {
		this.logger.log(
			`Миграция данных с версии ${fromVersion} на ${this.currentSchemaVersion}`,
		);
		...
		for (const migration of migrationPath) {
			this.logger.log(
				`Применение миграции: ${migration.from} -> ${migration.to}`,
			);
		...
		this.logger.log(
			`Миграция завершена: ${fromVersion} -> ${this.currentSchemaVersion}`,
		);
		return migrated;
	}
	...
	private applyMigration(
		...
		switch (`${fromVersion}-${toVersion}`) {
			...
			default:
				this.logger.warn(
					`Неизвестный путь миграции: ${fromVersion} -> ${toVersion}`,
				);
				return migrated;
		}
	}
