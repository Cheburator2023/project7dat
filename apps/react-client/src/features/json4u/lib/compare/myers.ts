import { type Diff, newDiff } from "./diff";
	...
	while (i < diffs.length) {
		...
		if (diff?.type === "del" && next?.type === "ins") {
			...
			for (
				let j = 0;
				j < n &&
				a[diff.offset + diff.length - 1] === b[next.offset + next.length - 1];
				j++
			) {
				diff.length--;
				next.length--;
...
function diff(
	...
	const oldArray = options?.isArrayDiff
		? oldString.split("")
		: tokenize(oldString).filter((c) => c);
	const newArray = options?.isArrayDiff
		? newString.split("")
		: tokenize(newString).filter((c) => c);

	const newLen = newArray.length;
	const oldLen = oldArray.length;
	let editLength = 1;
	let maxEditLength = newLen + oldLen;
	maxEditLength = Math.min(
		maxEditLength,
		options?.maxEditLength || maxEditLength,
	);
	const bestPath: (Path | undefined)[] = [
		{ newPos: -1, lastComponent: undefined },
	];
...
function buildValues(
	...
	let currentComponent = lastComponent;
	while (currentComponent) {
		components.push(currentComponent);
		nextComponent = currentComponent.previousComponent;
		// biome-ignore lint/performance/noDelete: <explanation>
		delete currentComponent.previousComponent;
