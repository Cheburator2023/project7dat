...
import type { ParsedTree } from "@react-client/features/json4u/lib/worker/command/parse";
import { getEditorState } from "@react-client/features/json4u/stores/editorStore";
import { getStatusState } from "@react-client/features/json4u/stores/statusStore";
import { getTreeState } from "@react-client/features/json4u/stores/treeStore";
import { type DebouncedFunc, debounce } from "lodash-es";
import type { IScrollEvent, editorApi } from "./types";
...
export class EditorWrapper {
	...
	delayParseAndSet: DebouncedFunc<
		(text: string, extraOptions: ParseOptions, resetCursor: boolean) => void
	>;
	...
	range(offset: number, length: number) {
		return window.monacoApi.RangeFromPositions(
			this.getPositionAt(offset),
			this.getPositionAt(offset + length),
		);
	}

	revealPosition(lineNumber: number, column = 1, focus = true) {
		if (focus) {
			this.editor.focus();
		}
	...
	setTree({ treeObject }: ParsedTree, resetCursor = true) {
		...
		const onDidChangeCursorPosition = debounce(
			(e) => {
				...
				const selectionLength =
					model?.getValueInRange(this.editor.getSelection()!).length ?? 0;
	...
	listenOnKeyDown() {
		this.editor.onKeyDown((e) => {
			if (
				e.keyCode === window.monacoApi.KeyCode.KeyK &&
				(e.metaKey || e.ctrlKey)
			) {
				e.preventDefault();
				e.stopPropagation();
				...
				e.keyCode === window.monacoApi.KeyCode.Enter &&
				(e.metaKey || e.ctrlKey)
			) {
				e.preventDefault();
				e.stopPropagation();
