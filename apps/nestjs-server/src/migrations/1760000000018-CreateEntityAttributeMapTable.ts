...
export class CreateEntityAttributeMapTable1760000000018 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(`
			CREATE TABLE IF NOT EXISTS entity_attribute_map (
				...
				CONSTRAINT fk_entity_attribute_map_deptype FOREIGN KEY (deptype_id) REFERENCES dependency_type(deptype_id)
				)
		`);
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(`DROP TABLE IF EXISTS entity_attribute_map`);
	}
}
